...
import { useHomeStore, InterviewQuestion } from "@/store/homeStore";
import { motion, AnimatePresence } from "framer-motion";
import { Calendar, Search, ChevronLeft, ChevronRight, ArrowLeft } from "lucide-react";
...
function getQuestionCategory(q: InterviewQuestion): string {
    const title = q.title.toLowerCase();
    const keywords = (q.keywords || "").toLowerCase();
    ...
    const contentText = (q.content || "").toLowerCase().substring(0, 1000);
    for (const cat of CATEGORIES) {
...
export default function InterviewQuestionsListing() {
    ...
    useEffect(() => {
        fetchPublicInterviewQuestions(1, 100);
    }, [fetchPublicInterviewQuestions]);

    const allQuestions = interviewQuestionsPage?.items || [];

    const filteredQuestions = allQuestions.filter((q) => {
        const matchesCategory = selectedCategory ? getQuestionCategory(q) === selectedCategory : true;
        const matchesSearch = q.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                              (q.keywords || "").toLowerCase().includes(searchQuery.toLowerCase());
        return matchesCategory && matchesSearch;
    });

    const questionsByCategory = CATEGORIES.reduce((acc, cat) => {
        const qInCat = allQuestions.filter(q => getQuestionCategory(q) === cat.name && 
    ...
    const categoryCounts = CATEGORIES.reduce((acc, cat) => {
        const count = allQuestions.filter(q => getQuestionCategory(q) === cat.name).length;
        acc[cat.name] = count;
        return acc;
    }, {} as Record<string, number>);

    const totalAllCount = allQuestions.length;
    ...
    return (
        <main className="min-h-screen pt-32 pb-24 bg-slate-50">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 border-b border-slate-200 pb-8">
                    <div>
                        ...
                        <p className="mt-2 text-slate-600">
                            Prepare for your next tech interview with our curated list of commonly asked questions and expert answers.
                        </p>
                    </div>
                ...
                {loading.interviewQuestions && allQuestions.length === 0 ? (
                                ...
                                {selectedCategory === null ? (
                                    ...
                                    >
                                        {Object.keys(questionsByCategory).length === 0 ? (
                                            ...
                                            CATEGORIES.map((cat) => {
                                                const questions = questionsByCategory[cat.name] || [];
...
function QuestionCard({ question }: { question: InterviewQuestion }) {
    return (
        ...
        >
            <Link href={`/interview-questions/${question.slug}`} className="flex flex-col h-full">
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                    <img
                        src={question.featureImage || "https://images.unsplash.com/photo-1573497620053-ea5300f94f21?w=800&q=80"}
                        alt={question.title}
