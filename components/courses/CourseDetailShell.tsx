...
export default function CourseDetailShell({ slug, initialCategories }: CourseDetailShellProps) {
  ...
  useEffect(() => {
    if (categories.length === 0 && initialCategories && initialCategories.length > 0) {
      useHomeStore.setState({ categories: initialCategories });
  ...
  const isCategory = activeCategories.some((c) => c.slug === slug);

  if (loadingCategories && activeCategories.length === 0) {
    return (
  ...
  if (isCategory) {
    return <CategoryView key={`cat-${slug}`} slug={slug} initialCategories={activeCategories} />;
  }
...
function CategoryView({ slug, initialCategories }: { slug: string; initialCategories: any[] }) {
  ...
  const currentCategory = initialCategories.find((c) => c.slug === slug);
