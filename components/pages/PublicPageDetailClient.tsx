...
import { useHomeStore, PublicPage } from "@/store/homeStore";
import Link from "next/link";
import { ChevronRight } from "lucide-react";

interface Props {
  slug: string;
  initialPages: PublicPage[];
}

export default function PublicPageDetailClient({ slug, initialPages }: Props) {
  ...
  useEffect(() => {
    if (publicPages.length === 0 && initialPages && initialPages.length > 0) {
      useHomeStore.setState({ publicPages: initialPages });
    } else if (publicPages.length === 0) {
      fetchPublicPages();
    }
  }, [initialPages, publicPages]);

  const activePages = publicPages.length > 0 ? publicPages : initialPages;
